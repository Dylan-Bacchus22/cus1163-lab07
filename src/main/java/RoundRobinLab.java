...
public class RoundRobinLab {
    ...
    public static void scheduleRoundRobin(List<Process> processes, int timeQuantum) {
    		int currentTime = 0;
        	ArrayList<Process> readyQueue = new ArrayList<Process>(); //creates the ready queue
        	
        	for (Process p : processes) { //adds processes to queue
        		readyQueue.add(p);
        	}
        	
        	while(!readyQueue.isEmpty()) { //runs as long as queue is not empty
        		Process current = readyQueue.remove(0); //removes first process from queue
        		int executeTime = Math.min(timeQuantum, current.remainingTime);
        		currentTime += executeTime;
        		current.remainingTime -= executeTime;
        		
        		if(current.remainingTime > 0) {
        			readyQueue.add(current); //if not done, add it back to queue
        		}
        		else {
        			current.completionTime = currentTime; //when done record completion time
        		}
        	}
